...
public class DazeStatus : SpellStatus
{
	...
	public DazeStatus(MobileEntity entity, int ticks, TimeSpan delay) : base(entity)
	{
		_ticks = ticks;
		_delay = delay;
	}
	...
	private void Announce()
	{
		...
		foreach (var player in beholders.SelectMany(g => g.Members))
			player.SendCombatMessage(6300043, _entity.Name);
	}
	...
	private void OnTick()
	{
		if (_entity != null)
		{
			if (!_entity.Deleted && _entity.IsAlive && _ticks > 0)
			{
				...
				if (_entity.GetStatus<StunStatus>() is StunStatus stunStatus)
					_entity.RemoveStatus(stunStatus);

				if (_entity is PlayerEntity)
					_entity.SendCombatMessage(6300042);
