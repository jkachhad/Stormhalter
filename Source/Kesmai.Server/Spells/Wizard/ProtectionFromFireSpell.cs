...
public class ProtectionFromFireSpell : DelayedSpell
{
	private static SpellInfo _info = new SpellInfo(43, "Protection from Fire", typeof(ProtectionFromFireSpell), 4);
	...
	public void CastAt(MobileEntity target)
	{
		...
		if (CheckSequence())
		{
			...
			if (!target.GetStatus(typeof(FireProtectionStatus), out var status))
			{
				status = new FireProtectionStatus(target)
				{
			...
			if (_caster != target || _item != null)
				target.SendLocalizedMessage(6300311, 543);
		}
		else
		{
			Fizzle();
	...
	{
		private ProtectionFromFireSpell _spell;

		public InternalTarget(ProtectionFromFireSpell spell) : base(flags: TargetFlags.Beneficial)
		{
			_spell = spell;
		}
