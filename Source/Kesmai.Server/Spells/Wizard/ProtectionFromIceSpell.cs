...
public class ProtectionFromIceSpell : DelayedSpell
{
	private static SpellInfo _info = new SpellInfo(42, "Protection from Ice", typeof(ProtectionFromIceSpell), 5);
	...
	public void CastAt(MobileEntity target)
	{
		...
		if (CheckSequence())
		{
			...
			if (!target.GetStatus(typeof(IceProtectionStatus), out var status))
			{
				status = new IceProtectionStatus(target)
				{
			...
			if (_caster != target || _item != null)
				target.SendLocalizedMessage(6300311, 542);
		}
		else
		{
			Fizzle();
	...
	{
		private ProtectionFromIceSpell _spell;

		public InternalTarget(ProtectionFromIceSpell spell) : base(flags: TargetFlags.Beneficial)
		{
			_spell = spell;
		}
