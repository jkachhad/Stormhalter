...
public class LightningResistanceSpell : DelayedSpell
{
	private static SpellInfo _info = new SpellInfo(50, "Lightning Resistance", typeof(LightningResistanceSpell), 13);
	...
	public virtual void CastAt(MobileEntity target)
	{
		...
		if (CheckSequence())
		{
			...
			if (!target.GetStatus(typeof(LightningResistanceStatus), out var status))
			{
				status = new LightningResistanceStatus(target)
				{
			...
			if (_caster != target || _item != null)
				target.SendLocalizedMessage(6300311, 550);
		}
		else
		{
			Fizzle();
	...
	{
		private LightningResistanceSpell _spell;

		public InternalTarget(LightningResistanceSpell spell) : base(flags: TargetFlags.Beneficial)
		{
			_spell = spell;
		}
