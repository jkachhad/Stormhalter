...
public class FearResistanceSpell : DelayedSpell
{
	private static SpellInfo _info = new SpellInfo(49, "Fear Resistance", typeof(FearResistanceSpell), 9);
	...
	public virtual void CastAt(MobileEntity target)
	{
		...
		if (CheckSequence())
		{
			...
			if (!target.GetStatus(typeof(FearResistanceStatus), out var status))
			{
				status = new FearResistanceStatus(target)
				{
			...
			if (_caster != target || _item != null)
				target.SendLocalizedMessage(6300311, 549);
		}
		else
		{
			Fizzle();
	...
	{
		private FearResistanceSpell _spell;

		public InternalTarget(FearResistanceSpell spell) : base(flags: TargetFlags.Beneficial)
		{
			_spell = spell;
		}
