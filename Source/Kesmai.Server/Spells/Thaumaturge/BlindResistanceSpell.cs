...
public class BlindResistanceSpell : DelayedSpell
{
	private static SpellInfo _info = new SpellInfo(47, "Blind Resistance", typeof(BlindResistanceSpell), 11);
	...
	public virtual void CastAt(MobileEntity target)
	{
		...
		if (CheckSequence())
		{
			...
			if (!target.GetStatus(typeof(BlindResistanceStatus), out var status))
			{
				status = new BlindResistanceStatus(target)
				{
			...
			if (_caster != target || _item != null)
				target.SendLocalizedMessage(6300311, 547);
		}
		else
		{
			Fizzle();
	...
	{
		private BlindResistanceSpell _spell;

		public InternalTarget(BlindResistanceSpell spell) : base(flags: TargetFlags.Beneficial)
		{
			_spell = spell;
		}
