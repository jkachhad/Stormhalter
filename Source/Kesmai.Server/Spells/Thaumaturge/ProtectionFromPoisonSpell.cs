...
public class ProtectionFromPoisonSpell : DelayedSpell
{
	private static SpellInfo _info = new SpellInfo(84, "Protection from Poison", typeof(ProtectionFromPoisonSpell), 21);
	...
	public void CastAt(MobileEntity target)
	{
		...
		if (CheckSequence())
		{
			...
			if (!target.GetStatus(typeof(PoisonProtectionStatus), out var status))
			{
				status = new PoisonProtectionStatus(target)
				{
			...
			if (_caster != target || _item != null)
				target.SendLocalizedMessage(6300311, 584);
		}
		else
		{
			Fizzle();
	...
	{
		private ProtectionFromPoisonSpell _spell;

		public InternalTarget(ProtectionFromPoisonSpell spell) : base(flags: TargetFlags.Beneficial)
		{
			_spell = spell;
		}
