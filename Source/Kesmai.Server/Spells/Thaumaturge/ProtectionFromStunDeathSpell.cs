...
public class ProtectionFromStunDeathSpell : DelayedSpell
{
	private static SpellInfo _info = new SpellInfo(45, "Protection from Stun and Death", typeof(ProtectionFromStunDeathSpell), 52);
	...
	public virtual void CastAt(MobileEntity target)
	{
		...
		if (CheckSequence())
		{
			...
			if (!target.GetStatus(typeof(StunDeathProtectionStatus), out var status))
			{
				status = new StunDeathProtectionStatus(target)
				{
			...
			if (_caster != target || _item != null)
				target.SendLocalizedMessage(6300311, 545);
		}
		else
		{
			Fizzle();
	...
	{
		private ProtectionFromStunDeathSpell _spell;

		public InternalTarget(ProtectionFromStunDeathSpell spell) : base(flags: TargetFlags.Beneficial)
		{
			_spell = spell;
		}
