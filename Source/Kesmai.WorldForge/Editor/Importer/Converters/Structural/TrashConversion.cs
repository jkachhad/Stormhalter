...
namespace Kesmai.WorldForge.Editor
{
	...
	public class TrashConversion : ConversionPass
	{
		private List<int> _targets;

		/// <summary>
		/// Initializes a new instance of the <see cref="TrashConversion"/> class.
		/// </summary>
		public TrashConversion(List<int> targets)
		{
			_targets = targets;
		}
		...
		public void Process(ISegmentImporter importer, IImportedRegion convertibleRegion, SegmentRegion convertedRegion)
		{
			...
			foreach (var tile in matches)
			{
				...
				if (staticComponents.Count() > 1)
					throw new Exception("Encountered multiple trash candidates.");

				foreach (var component in staticComponents)
					tile.ReplaceComponent(component, new TrashComponent(component.Static));
			}
		}
	}
