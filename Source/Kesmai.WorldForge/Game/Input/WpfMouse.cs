...
public class WpfMouse
{
    ...
    private void HandleMouse(object sender, MouseEventArgs e)
    {
        ...
        var pos = Mouse.GetPosition(_focusElement);
        ...
        if (!CaptureMouseWithin)
        {
            ...
            pos = new Point(Clamp(pos.X, 0, _focusElement.RenderSize.Width),
                Clamp(pos.Y, 0, _focusElement.RenderSize.Height));
        }
            ...
            if (_focusElement.IsMouseCaptured)
            {
                ...
                if (!hit)
                {
                    ...
                    _mouseState = new MouseState(_mouseState.X, _mouseState.Y, _mouseState.ScrollWheelValue,
                        (ButtonState) e.LeftButton, (ButtonState) e.MiddleButton, (ButtonState) e.RightButton,
                        (ButtonState) e.XButton1,
        ...
        if (CaptureMouseWithin)
        {
            ...
            if (!_focusElement.IsMouseCaptured)
            {
                ...
                if (IsControlOnActiveWindow(_focusElement))
                {
        ...
        {
            if (_focusElement.IsFocused && !IsControlOnActiveWindow(_focusElement))
            {
                // don't update mouse events if we are just hovering over different window
                return;
            }
