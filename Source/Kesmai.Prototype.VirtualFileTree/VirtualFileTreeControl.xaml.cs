...
namespace Kesmai.Prototype.VirtualFileTree
{
    public partial class VirtualFileTreeControl : UserControl
    {
        public static readonly DependencyProperty SolutionProperty =
            DependencyProperty.Register(nameof(Solution), typeof(SegmentSolution), typeof(VirtualFileTreeControl),
                new PropertyMetadata(null, OnSolutionChanged));

        public SegmentSolution? Solution
        {
            get => (SegmentSolution?)GetValue(SolutionProperty);
        ...
        private static void OnSolutionChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (e.OldValue is SegmentSolution oldSolution)
            {
            ...
            if (e.NewValue is SegmentSolution newSolution)
            {
        ...
        {
            if (e.PropertyName == nameof(SegmentSolution.RootPath))
            {
                SetupWatcher();
                LoadRoot();
            }
            else if (e.PropertyName == nameof(SegmentSolution.Name))
            {
                LoadRoot();
            }
        }
        ...
        private void LoadRoot()
        {
            ...
            var rootPath = Solution.RootPath;
            var rootItem = new TreeViewItem { Tag = rootPath };
            rootItem.Header = CreateHeader(Solution.Name, rootPath, true);
            ...
            if (!string.IsNullOrEmpty(rootPath) && Directory.Exists(rootPath))
            {
                ...
                foreach (var file in Solution.VirtualFiles)
